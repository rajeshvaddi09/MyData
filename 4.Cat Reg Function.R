...
Yord  <- cut(Ycont, breaks=quantile(Ycont), include.lowest=TRUE,
             labels=c("A", "B", "C", "D"), ordered=FALSE)
...
vglmFit <- vglm(Formula, family=multinomial(refLevel=1), Dataset=Dataset)
#Predict The Order
...
vglm0 <- vglm(Formula, family=multinomial(refLevel=1), data=Dataset)
